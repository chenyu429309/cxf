	...
	<div class="col-md-12">
		...
		<section class="tile">
			...
			<div class="tile-header">
				...
				<button style="float: right;" id="newUser">NEW</button>
			</div>
			<!-- /tile header -->
			<!-- tile body -->
				...
				<table class="table table-bordered table-sortable">
					<thead>
						<tr>
							...
							<th>角色</th>
							<th>是否在职</th>
							<th><span class="glyphicon glyphicon-log-in"></span>添加角色</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${pageList.recordList}" var="user"
							...
							<tr>
								<td>${index}</td>
								<td>${user.ChinaName }</td>
								<td>${user.department.name }</td>
								<td><c:forEach items="${ user.roles}" var="role">
                       					${role.name};
                       					</c:forEach> ${user.department.name }</td>
								<td>${user.living }</td>
								<td><a href="#" class="btn">添加</a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
...
<script
	src="${pageContext.request.contextPath}/assets/js/userManager_tables.js"></script>
